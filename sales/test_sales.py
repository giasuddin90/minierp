"""
Comprehensive test cases for the Sales module
...
class SalesModelTests(TestCase):
    ...
    def test_mark_delivered_regular_sale(self):
        ...
        self.assertEqual(stock.current_stock, Decimal('99.00'))
    ...
    def test_cancel_delivered_order(self):
        ...
        stock = Stock.objects.get(product=self.product)
        self.assertEqual(stock.current_stock, Decimal('101.00'))
...
class SalesViewTests(TestCase):
    ...
    def test_sales_order_create_post(self):
        """Test creating a sales order via POST"""
        form_data = {
            'sales_type': 'regular',
            ...
            'form-0-total_price': '200.00',
        }
        
        response = self.client.post(reverse('sales:order_create'), form_data)
        self.assertEqual(response.status_code, 302)  # Redirect after successful creation
        
        # Check order was created
        order = SalesOrder.objects.get(order_number__startswith='SO-')
        self.assertEqual(order.sales_type, 'regular')
        self.assertEqual(order.customer, self.customer)
    ...
    def test_instant_sales_create_post(self):
        """Test creating an instant sale via POST"""
        form_data = {
            'customer_name': 'Anonymous Customer',
            'order_date': date.today().strftime('%Y-%m-%d'),
            ...
            'form-0-quantity': '1',
            'form-0-unit_price': '100.00',
            'form-0-total_price': '100.00',
        }
        
        response = self.client.post(reverse('sales:instant_sales'), form_data)
        self.assertEqual(response.status_code, 302)  # Redirect after successful creation
        
        # Check instant sale was created
        order = SalesOrder.objects.get(order_number__startswith='IS-')
        self.assertEqual(order.sales_type, 'instant')
        self.assertEqual(order.customer_name, 'Anonymous Customer')
        self.assertEqual(order.status, 'delivered')
        self.assertEqual(order.items.count(), 1)
...
class SalesIntegrationTests(TestCase):
    ...
    def test_complete_sales_workflow(self):
        ...
        stock = Stock.objects.get(product=self.product)
        self.assertEqual(stock.current_stock, Decimal('98.00'))  # 100 - 2
    ...
    def test_instant_sales_workflow(self):
        ...
        # Check stock was reduced
        stock = Stock.objects.get(product=self.product)
        self.assertEqual(stock.current_stock, Decimal('99.00'))  # 100 - 1
    ...
    def test_cancel_delivered_order_workflow(self):
        ...
        stock = Stock.objects.get(product=self.product)
        self.assertEqual(stock.current_stock, Decimal('101.00'))  # 100 + 1 (restored)
...
if __name__ == '__main__':
    ...
    failures = test_runner.run_tests(['sales.test_sales'])
